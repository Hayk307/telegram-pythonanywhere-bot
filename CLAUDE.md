...
│   ├── slides.py         # /slides engine — AI JSON spec → themed .pptx (python-pptx) or .pdf (fpdf2); libs imported lazily
│   └── assets/
│       └── fonts/        # Bundled DejaVu Sans TTFs — Unicode font so PDF export renders Cyrillic (not "?")
├── tests/
...
**Desktop "send photo, then command" flow.** On Telegram *mobile* users add a caption to a photo (e.g. `/edit …`), so the command and image arrive together. On Telegram *Desktop* the natural drag-drop flow is to send the photo with **no caption** and then type the command as a **separate message** — which used to fail (the bot only accepted `/edit` as a caption or as a reply). `handle_file` now remembers each user's most recent photo (`store` key `lastphoto:{user_id}`, `LAST_PHOTO_TTL` = 1h), and `cmd_edit` falls back to that remembered photo when the message is neither a reply to a photo nor carries one. A bare captionless photo also gets a friendlier "now send /edit … or a format" hint. Degrades gracefully in stateless mode (`store is None` → no memory → same as the old caption/reply-only behavior).
...
`/slides [number] <topic> [pdf]` builds an attractive, downloadable deck on any topic — a **PowerPoint (.pptx) by default, or a PDF** if the request includes a format word (`pdf`, `as pdf`, `in pdf format`). An optional leading number (or a "N slides" phrase) asks for that many slides — e.g. `/slides 15 история интернета`, `/slides блокчейн, 20 slides`, or `/slides 12 фотосинтез pdf`; omit the number and the AI picks 5–8. `cmd_slides` in `bot/handlers.py` calls `_call_main()` **directly** (bypassing the per-user provider preference) because the deck needs structured JSON, which the optional HF/ArmGPT completion model can't produce. The AI is prompted to return a strict JSON spec (`title`, `subtitle`, `slides[].heading`, `slides[].bullets`) and to write all text **in Russian** with **accurate, non-fabricated** facts.

The engine lives in `bot/slides.py`:
- `parse_slides_request(text)` — pure stdlib. Splits the command argument into `(count | None, fmt, topic)`. `fmt` is `"pdf"` or `"pptx"` (default), set by a format word which is stripped from the topic; a leading integer or a "N slides" phrase sets the count. Testable without the render libs.
- `parse_deck_spec(raw)` — pure stdlib. Strips ```` ```json ```` fences and any prose around the object, extracts the outermost `{...}`, validates, and **clamps** to `MAX_SLIDES` (50) / `MAX_BULLETS` (6) so a runaway or over-large response can't produce a wall of text. Raises `SlideError` (user-facing) on anything unusable. Testable without the render libs installed.
- `build_deck(...)` → **.pptx** via **python-pptx**; `build_pdf(...)` → **.pdf** via **fpdf2**. Both render the same cohesive, modern theme (navy title slide with amber/azure accent strips; each content slide has a full-width navy header band with an amber top strip, an azure slide number, colored bullet dots, and a footer with the deck title + `n / total`). **Both libs are imported lazily inside their functions** (same crash-safety contract as `fileconvert.py`): `bot/slides.py` imports only stdlib at module load, so `bot.handlers` still boots if a render lib isn't installed — a `/slides` attempt just replies "not available (missing python-pptx / fpdf2)".

**Content richness + speaker notes.** The prompt asks for information-rich bullets (4–6 full, substantive sentences per slide, not fragments) plus a per-slide `notes` field (a 2–4 sentence speaker script). `parse_deck_spec` captures the optional `notes`; `build_deck` writes it to the slide's PPTX notes pane (`slide.notes_slide`). The PDF path renders slides only (PDF has no notes concept).

**Filling the slide (not clustering at the top).** Both renderers scale the bullet font up when there are few bullets (≈32pt for 1 → 20pt for 8+) and compute the inter-bullet gap so the bullets spread down the FULL content area instead of occupying the top half. The PDF path measures each bullet's wrapped height exactly (`multi_cell(dry_run=True, output=MethodReturnValue.HEIGHT)`); the PPTX path can't measure wrapping, so it estimates line count (avg glyph ≈ 0.5em) and reserves that height before distributing the leftover as equal gaps — this keeps long, wrapping bullets from overflowing past the footer while short bullets still spread out.

**Animations (PPTX only).** python-pptx has **no animation/transition API**, so `build_deck` injects raw OOXML: `_add_bullet_build()` writes a `<p:timing>` tree that fades each bullet in one-by-one on click (a "Fade / By paragraph" entrance targeting the bullet text box by its `shape_id`, with a matching `<p:bldLst>`), and `_add_fade_transition()` adds a `<p:fade>` slide transition. These are appended to the slide's `<p:sld>` element (schema child order is `cSld, clrMapOvr, transition, timing`, so appending after the existing children is correct). Hand-written timing is fragile — malformed XML makes PowerPoint silently drop the animation or reject the file — so `test_slides.py` round-trips the deck (re-opens it, asserts `p:timing`/`clickEffect`/`p:fade`/`p:bldP` and a valid notes slide). **PDFs cannot animate elements** — the `/slides … pdf` path is static by design.

**PDF + Cyrillic:** fpdf2's built-in core fonts are Latin-1 only, so Russian text would become `?`. `build_pdf` registers a bundled **DejaVu Sans** TTF (`bot/assets/fonts/`, freely redistributable) via `add_font()`, which embeds the glyphs so Cyrillic renders. There is **no PPTX→PDF conversion step** (that would need LibreOffice, which the PA free tier lacks) — the PDF is rendered directly from the same spec. Neither `google`/CLI tools nor a new PA outbound-whitelist entry are needed.
