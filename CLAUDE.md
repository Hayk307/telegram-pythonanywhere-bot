...
| `GEMINI_API_KEY` | No | _falls back to `GOOGLE_API_KEY`_ | Enables the `/edit` command (edit a photo with a text prompt) via Google Gemini's image model ("Nano Banana"). The key must have the **Generative Language API** enabled — that's a different API from the Custom Search API `/image` uses, so a Programmable-Search-only key won't work for `/edit`. Get one at aistudio.google.com/apikey (free tier). When unset (and `GOOGLE_API_KEY` also unset), `/edit` replies that editing is off. Requires `generativelanguage.googleapis.com` on the PA outbound whitelist |
| `GEMINI_IMAGE_MODEL` | No | `gemini-2.5-flash-image` | Gemini image model id used by `/edit`. Override if Google renames the model or you want a different image model |
