...
| `HF_EDIT_SPACE_ID` | No | `black-forest-labs/FLUX.1-Kontext-Dev` | Hugging Face Space that powers the `/edit` command (edit a photo from a text prompt). **Free, no key required** — called via `gradio_client`. The default is Flux.1 Kontext, whose `/infer` endpoint takes `(input_image, prompt, seed, randomize_seed, guidance_scale, steps)` and returns a WebP (the bot converts it to JPEG via Pillow so Telegram renders it inline). Override to use a different editing Space, but confirm it exposes a compatible `/infer` signature or adapt `_edit_image` in `bot/handlers.py`. On PA free tier, `/edit` needs `*.hf.space` on the outbound whitelist (works locally without it) |
| `HF_TOKEN` | No | — | HF auth token. Optional for `/edit`: raises the anonymous per-IP quota on the editing Space (get a free token at huggingface.co/settings/tokens). Also used by the optional HF chat provider when its Gradio space is private/gated |
